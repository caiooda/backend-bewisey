...
import ObterMateriaUseCase from "../../../application/usecases/materia/ObterMateriaUseCase";
...
export class ObterMateriaController {
  async handle(request: Request, response: Response): Promise<Response> {
    try {
      const databaseFactory = new DatabaseRepositoryFactory(connection);
      const service = new ObterMateriaUseCase(databaseFactory);
      const result = await service.execute();
      return response.status(200).json(result);
    } catch (erro: any) {
      const controller = HandleErroController(
        erro,
        "Não foi possível carregar lista de Matérias."
      );
